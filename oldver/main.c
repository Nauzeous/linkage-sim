...
Vec2 vec2_mult(Vec2 a, float b){return (Vec2){a.x*b,a.y*b};}
...
bool hovering_over_line(Vec2 mouse_pos, Graph* graph,Line line){
	...
	Vec2 p1 = line.source->pos;
	Vec2 p2 = line.target->pos;
...
void draw_view(Graph* graph,State* state,Vec2 mouse_pos){
	for (int i = 0; i < graph->num_lines;i++){
		Colour line_col = (state->hovering_line && graph->lines+i==state->line_hovered)?BLUE:BLACK;
		Vec2 p1 = graph->lines[i].source->pos;
		Vec2 p2 = graph->lines[i].target->pos;
		DrawLineEx(p1,p2,5.0f,line_col);
	}

	if (state->hovering_joint){
		Node* hj = state->joint_hovered;
		for(int i = 0;i<hj->num_connected;i++){
			Line* cl = hj->connected[i];
			DrawLineEx(cl->source->pos,cl->target->pos,5.0f,RED);
		}
	}

	if (state->selected_joint == true){
		DrawLineEx(state->joint_selected->pos,mouse_pos,5.0f,RED);
	}

	for (int i = 0;i<graph->num_nodes;i++){
		Colour joint_col = (state->hovering_joint && graph->nodes+i==state->joint_hovered)?BLUE:BLACK;
...
void get_position(Node* node, Graph* graph) {
    switch (node->state) {
        case Fixed:
            return;
        case Rotating:
    ...
    for(int i = 0; i < node->num_connected; i++) {
        ...
        Node* other_node = (curr_line->source == node) ? curr_line->target : curr_line->source;
        
        // Only consider connections to known/fixed nodes
        if (other_node->state != Unknown) {
            if (conn1 == NULL) {
                conn1 = curr_line;
                pnode1 = other_node;
            } else {
                conn2 = curr_line;
                pnode2 = other_node;
                break; // We found our two connections
            }
        }
    }
...
void add_node(Graph* graph,Vec2 pos,Node_state state){
	Node new_node;
	new_node.pos = pos;
	new_node.connected = malloc(sizeof(Line)*MAX_CONNECTIONS);
	new_node.num_connected = 0;
	new_node.state = state;
	graph->nodes[graph->num_nodes++] = new_node;
}

void add_line(Graph* graph,Node* a,Node* b){
	float len = Vector2Length(Vector2Subtract(b->pos,a->pos));
	Line new_line = {len,a,b};
	if (line_exists(graph,new_line))return;
	Line* line_ptr = graph->lines + graph->num_lines;
	graph->num_lines++;
	a->connected[a->num_connected]=line_ptr;
	b->connected[b->num_connected]=line_ptr;
	a->num_connected++;
	b->num_connected++;
	*line_ptr = new_line;
}
...
void remove_connection(Graph* graph,Line* line){	
	// node is start point, node2 is end point of connection

	// TODO: make this update the pointers in the original nodes connected by the line
	graph->num_lines--;
	*line = graph->lines[graph->num_lines];
}
